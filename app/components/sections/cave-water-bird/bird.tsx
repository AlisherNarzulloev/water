...
export default function BirdSection({ scrollYProgress }: BirdSectionProps) {
  ...
  return (
    <div className="h-[200vh]">
      <motion.h1
        style={{ opacity, y }}
        className="fixed top-10 left-1/2 xl:-mt-10 -translate-x-1/2 text-white text-[85px] xl:text-[8.5vw] font-extrabold tracking-widest select-none pointer-events-none leading-none z-0"
      >
        СБАЛАНСИРОВАННАЯ
      </motion.h1>
      <Image
        ...
        height={100}
        src={"/bird.webp"}
        className="absolute bottom-96 2xl:-top-96 scale-125 xl:-bottom-44 z-10 w-full h-fit"
        alt="bird"
        unoptimized
      />
      <div className="fixed top-0 z-20 h-full pb-12 w-full px-8">
        <motion.p
          style={{ opacity, y }}
          className="self-end max-w-xl 2xl:max-w-2/5 font-bold text-right absolute top-1/2 -translate-y-1/2 right-8 z-10 p-4 backdrop-blur-[6px] rounded-2xl text-white text-lg 2xl:text-3xl"
        >
        ...
        <motion.p
          style={{ opacity, y }}
          className="max-w-2xl 2xl:max-w-2/5 self-start text-left absolute bottom-8 left-8 z-10 p-4 backdrop-blur-[6px] rounded-2xl font-bold text-white text-lg 2xl:text-3xl"
        >
