...
export default function CaveSection({ scrollYProgress }: CaveSectionProps) {
  ...
  return (
    <div className="h-1/3 relative w-full">
      <motion.div
        style={{ opacity, y }}
        className="fixed top-0 xl:-top-12 1xl:top-0 2xl:-top-14 w-full text-center"
      >
        <Heading>ЛЕГКАЯ</Heading>
      </motion.div>
      <motion.div
        style={{ opacity, y }}
        className="self-end fixed top-1/2 -translate-y-1/2 right-8 max-w-2/3 xl:max-w-3/5 text-right blur-blue text-white"
      >
      ...
      <motion.p
        style={{ opacity, y }}
        className="fixed bottom-14 left-8 max-w-3/5 xl:max-w-2/3 text-clamp self-start blur-blue font-bold text-white"
      >
