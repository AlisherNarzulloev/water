  ...
  return (
    <div className="h-screen relative top-0 z-20">
      ...
      <div className="h-screen w-full">
        ...
        >
          <div className="self-end max-w-3/5 absolute top-1/2 -translate-y-1/2 left-4 xl:left-8 blur-gray -mt-16 xl:mt-7">
            <p className="font-bold title-clamp">
              Локальность как принцип. Независимость как выбор.
            </p>
            <p className="text-base text-clamp mt-4">
              ...
              производственной цепочки до упаковки. И в этом коротком списке он
              занимает уверенную вторую позицию. Легче воды TURAN только талые
              воды ледников Антарктики. Но в отличие от лидера списка TURAN
              доступен всем желающим.
            </p>
          </div>
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-full px-8 grid grid-cols-3 gap-4 xl:gap-8 z-30">
            {features.map((f) => (
              <div
                key={f.value}
                className="px-8 py-4 flex flex-col items-center text-white blur-gray"
              >
                <span className="title-clamp font-bold">{f.value}</span>
                <span className="text-clamp opacity-80 text-center font-medium leading-tight">
                  {f.label}
                </span>
              </div>
            ))}
